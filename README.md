...
<section id=""></secttion>
```
<li>The section function is to seperate the content of the website into parts, and we name each by by changing the id names. We could use the a href function to link the section by have the same name for id and href address, and the href="#name", # is essential when we want to link the section</li><br>

```
.functionname{}
class="functionname"
```
<li>The first row is based on css ( where we could change the type of the words), it will create a new function names functionames.<br>
After create it on css, then we could use it by adding class="functionname", for example:</li>

```
<p class="functionname">content</p>
```
<li>The p tag is an HTML element that represents a paragraph,by adding the class="functionname", we could change the type of words which is list in the special p function.</li>

```
<div></div>
```
<li>div doesn't represent anything, it just help us to group the content, and we could use the class function and id more convenient.</li>
<h2><ul><li>For editing the directory page</li></ul></h2>

<li>In the left hand side of the personal website, we have the button: Contact, education,work experience, publication and working papers.</li>
...
<li>In order to edit it, please go to the code line 19, where we could find the Navigation bar.</li><br>
...
<li>In order to add the new button, we just need to add an extra code between line 30 and 35.</li>

```
<li class="nav-item"><a class="nav-link js-scroll-trigger" href="#page-top"> Contact </a></li>
```
<li>Here, the only thing we need to change is href="#page-top"( which we will describe it later) and Contact ( what we name the button)</li><br>



<li>The order of the button is depends on the order of the code, so you could organize the order by change the order of the code.</li>

<h2><ul><li>Main content</li></ul></h2>
<li>Based on the code, we divided the code into 5 parts, and we use the code.</li>
...
```
<li>the section function is the way to seperate your whole content into several parts.</li><br>

<li>and the id is the default that to name the section, which could help us to use the buttion in the directory page.</li><br>
...
<li>Here, we named the section as "test1", and the href="#test1", and the button name is called TEST.
Everytime we change it, we have to make sure the button is connect to the section. Once the href equal to "#id", then the button and section part is matched.</li>
...
<li>Third is the number inside the span function, which represent the YEAR</li>
